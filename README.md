...
このプロジェクトは、Webブラウザ上で動作する**サイン波のパルス波形（断続的な音）ジェネレーター**です。  
任意の周波数、ゲイン、パルス時間（ON/OFF時間）を設定し、その場でプレビュー再生し、最終的なWAVファイルをダウンロードできます。
...
Geminiで生成しました。
手軽に任意で生成できる試験用の音声ファイルジェネレータとして使ってください。
